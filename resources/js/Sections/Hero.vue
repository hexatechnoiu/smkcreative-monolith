<template>
    <section id="hero" class="relative flex min-h-screen items-center justify-center bg-primary text-white">
        <div class="flex flex-col items-center justify-center px-8 py-24 text-center lg:container">
            <div aria-labelledby="content-left">
                <h1 class="text-3xl md:text-7xl font-bold text-yellow-400">
                    RPL PPLG
                </h1>
                <h2 class="text-2xl md:text-5xl font-medium">
                    SMKN NEGERI 2 SUMEDANG
                </h2>
            </div>
            <div
                class="absolute bottom-8 left-1/2 -translate-x-1/2 transform animate-pulse cursor-pointer text-white transition-all">
                Scroll down
            </div>
        </div>
    </section>
</template>